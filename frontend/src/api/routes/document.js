import { GET_DOCUMENTS_API, POST_DOCUMENTS_API, ANALYSER_IA_DOCUMENT_API, RECHERCHE_DOCUMENTS_API, SUGGERER_TRANSMISSION_DOCUMENT_API, SHARE_DOCUMENTS_API, FAVORITE_DOCUMENT_API, TRACK_DOCUMENTS_CONSULT_API, TRANSITION_DOCUMENT_API, RESOUDRE_COURRIER_DOCUMENT_API, RESOUDRE_QUALITE_DOCUMENT_API, HISTORIQUE_DOCUMENT_API, CONSULTATIONS_DOCUMENT_API, VERSIONS_DOCUMENT_API, NEW_VERSION_DOCUMENT_API, CORRIGER_ET_RENVOYER_DOCUMENT_API, VERIFIER_INTEGRITE_DOCUMENT_API, DOCUMENT_META_API, GET_PARTAGES_RECUS_API, MARQUER_PARTAGE_LU_API, MARQUER_TOUS_PARTAGES_LUS_API, UPDATE_DOCUMENTS_API, DELETE_DOCUMENTS_API, TRASH_DOCUMENTS_API, RESTORE_DOCUMENT_API, FORCE_DELETE_DOCUMENT_API, A_TRAITER_DOCUMENTS_API, COURRIER_COMPTEURS_DOCUMENTS_API, DECISION_CONGES_DOCUMENT_API, DECISION_PAIE_DOCUMENT_API, VERROUILLER_DOCUMENT_API, DEVERROUILLER_DOCUMENT_API, EDITION_WORD_DOCUMENT_API, ENREGISTRER_COPIE_WORD_DOCUMENT_API, DELETE_VERSION_DOCUMENT_API } from "..";
...
/**
 * Supprime une ancienne version (fichier + ligne d'historique) — pour garder
 * un historique gérable sans attendre l'élagage automatique.
 * @param {Number} id
 * @param {Number} versionId
 */
export async function deleteVersion(id, versionId){
    const {url,...meta} = DELETE_VERSION_DOCUMENT_API;
    return await fetch(url+`/${id}/versions/${versionId}`, {...meta,credentials:'include'})
}
...
/**
 * Résout un document Qualité & Risque ("Lu et approuvé"/"Lu et rejeté").
 * @param {Number} id
 * @param {{decision: 'approuve'|'rejete'}} data
 */
export async function resoudreQualite(id, data){
    const {url,...meta} = RESOUDRE_QUALITE_DOCUMENT_API;
    return await fetch(url+`/${id}/qualite/resoudre`, {...meta,body:JSON.stringify(data),credentials:'include'})
}
...
export async function marquerPartageLu(id){
    const {url,...meta} = MARQUER_PARTAGE_LU_API;
    return await fetch(url+`/${id}/marquer-lu`, {...meta,credentials:'include'})
}

export async function marquerTousPartagesLus(){
    const {url,...meta} = MARQUER_TOUS_PARTAGES_LUS_API;
    return await fetch(url, {...meta,credentials:'include'})
}
...
/**
 * Config signée (JWT) pour ouvrir un document .docx/.doc dans l'éditeur Word
 * en ligne (OnlyOffice) — pose au passage le même verrou pessimiste que
 * verrouillerDocument().
 * @param {Number} id
 */
export async function getEditionWordConfig(id){
    const {url,...meta} = EDITION_WORD_DOCUMENT_API;
    return await fetch(url+`/${id}/edition-word`, {...meta,credentials:'include'})
}
 ...
 * @param {Number} id
 * @param {{url: string, titre?: string}} data
 */
export async function enregistrerCopieWord(id, data){
    const {url,...meta} = ENREGISTRER_COPIE_WORD_DOCUMENT_API;
    return await fetch(url+`/${id}/enregistrer-copie-word`, {...meta,body:JSON.stringify(data),credentials:'include'})
}
